...
class TopicAnalyzer:
    """Extracts and merges topics from a document collection.

    Supports two extraction modes: LLM-based (via Ollama) and statistical
    (TF-IDF). Topics are deduplicated using n-gram similarity before merging
    with existing topic lists.

    Class constants:
        MAX_TOPICS: Maximum number of topics to return
        TOPIC_THRESHOLD: Minimum score threshold for topic inclusion
    ...
    def __init__(self, min_freq: int = 2):
        ...
        Args:
            min_freq: Minimum term frequency for statistical extraction
        """
    ...
    def extract_topics(
        ...
        """Extract topics from documents, preferring LLM when available.

        Args:
            documents: List of document texts
            use_llm: Whether to attempt LLM extraction first
    ...
    def _llm_extract_topics(self, texts: List[str]) -> List[Tuple[str, float]]:
        try:
            import httpx
            from rag.ollama_utils import get_default_llm_from_registry
    ...
    def _tokenize_text(self, text: str) -> List[str]:
        """Tokenize text into normalized terms (Chinese + English).

        Handles Chinese character sequences (2+ chars) and English
        alphanumeric tokens. Removes stopwords and garbage tokens.
    ...
    def _stat_extract_topics(self, texts: List[str]) -> List[Tuple[str, float]]:
        ...
        Computes term frequency across documents and penalizes common terms
        using IDF weighting. Returns top-scoring terms as topics.
    ...
    def merge_topics(
        ...
        """Merge new topics with existing ones, deduplicating by similarity.

        Uses keyword similarity (n-gram Jaccard) to detect near-duplicates.
        Existing topics are preserved with reduced weight if not directly matched.

        Args:
            existing: Current topic list
            new_topics: Newly extracted (topic, score) tuples
            merge_weight: Weight applied to new topic scores during merge

        Returns:
            Deduplicated, merged topic list up to MAX_TOPICS items
        """
...
def get_kb_documents_for_analysis(kb_id: str, sample_size: int = 50) -> List[str]:
    """Retrieve text content from a knowledge base for topic analysis.

    Connects to the LanceDB table for the given KB and samples documents
...
def analyze_and_update_topics(
    ...
    """Analyze documents and update the KB's topic list if significantly changed.

    Extracts topics from sampled documents and merges with existing topics.
    Only updates the database if the Jaccard similarity indicates significant drift.

    Args:
        kb_id: Knowledge base identifier
        merge_weight: Weight for new topics when merging with existing
        has_new_docs: If False, skips extraction and returns existing topics

    Returns:
        Updated topic list (may be unchanged if not significantly different)
    """
    from kb_core.database import init_kb_meta_db
...
def _is_significant_change(
    ...
    """Check if topic change is significant using Jaccard similarity.

    Args:
        old: Previous topic list
        new: New topic list
        threshold: Jaccard similarity below which change is considered significant

    Returns:
        True if topics have changed beyond the threshold
    """
    if not old:
        return bool(new)
