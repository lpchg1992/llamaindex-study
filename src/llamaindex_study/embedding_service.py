...
class OllamaEmbeddingService:
    ...
    def _select_best_endpoint(self) -> OllamaEndpoint:
        ...
        # 按延迟排序
        available.sort(key=lambda x: x.avg_latency if x.avg_latency > 0 else float('inf'))
        return available[0]
    ...
    async def _embed_single(self, endpoint: OllamaEndpoint, text: str) -> List[float]:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                endpoint.avg_latency = (endpoint.avg_latency * endpoint.total_requests + latency) / (endpoint.total_requests + 1)
        ...
        for endpoint in sorted_endpoints:
            try:
                ...
                if has_error:
                    ...
                    logger.warning(f"端点 {endpoint.name} 部分失败: {len(failed_indices)}/{len(texts)}")
                    
                    if successful:
                        endpoint.avg_latency = (endpoint.avg_latency * endpoint.total_requests + total_latency) / (endpoint.total_requests + 1)
                        endpoint.total_requests += 1
                        return successful
