...
| `hybrid` | 向量检索 + 关键词全文搜索 | 查询包含专有名词、技术术语 |

**作用**：混合搜索结合向量语义检索和 LanceDB 原生全文搜索（FTS），通过数据库层融合结果（支持 relative_score 或 RRF）。
...
- 检索不到 → 使用 `retrieval_mode=hybrid` 启用混合搜索
