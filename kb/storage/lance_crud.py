...
class LanceCRUDService:
    ...
    def delete_by_chunk_ids(
        ...
        """Delete vector records by chunk IDs (alias for delete_by_node_ids).

        Args:
            kb_id: Knowledge base identifier
            chunk_ids: List of chunk/node IDs to delete
    ...
    def upsert_vector(
        ...
        Uses merge_insert with when_not_matched_insert/when_matched_update to
        perform an atomic upsert operation.

        Args:
            chunk_id: Unique chunk/node ID
            doc_id: Parent document ID
            vector: Embedding vector list
            kb_id: Knowledge base identifier (used if table_name not provided)
